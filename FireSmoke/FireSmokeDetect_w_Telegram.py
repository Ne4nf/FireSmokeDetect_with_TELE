...
smoke_alert_count   = 0
MAX_ALERTS          = 3
ALERT_INTERVAL      = 10.0  # giây
last_fire_alert     = 0.0
last_smoke_alert    = 0.0
MOTION_THRESHOLD    = 0.8 # Ngưỡng để nhận diện chuyển động
prev_frame_gray     = None   # Dùng cho so sánh chuyển động
...
# ======= 5. Gửi cảnh báo lên Telegram =======
def send_alert(frame, filename, caption):
    cv2.imwrite(filename, frame)
    with open(filename, "rb") as photo:
        bot.send_photo(CHAT_ID, photo, caption=caption)

# ======= 6. Phát hiện cháy và khói + lọc chuyển động =======
# ======= 6. Hàm detect chính =======
def detect_fire():
    ...
    while running and cap.isOpened():
        ...
        if prev_frame_gray is not None:
            ...
            motion_detected = True  # Frame đầu tiên giả định là có chuyển động

        prev_frame_gray = gray  # Cập nhật
        ...
        for result in results:
            for box in result.boxes:
                ...
                # Tính các đặc trưng trong vùng ROI
                ...
                # ---- SMOKE ----
                elif cls == 2 and conf > 0.55:
                    # Chỉ xác nhận khói nếu thỏa mãn các điều kiện
                    if mean_v < 200 and (mean_s > 60 or variance < 35):
...
# ======= 8. Chạy bot và phát hiện song song =======
def start_bot():
    bot.polling()
